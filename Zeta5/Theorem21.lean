...
* `Zeta5.QKM_natDegree`: the degree, from (2.9) (§2.3).
...
/-- (2.9): `Q_{K,M}` has degree exactly `h`. -/
theorem QKM_natDegree (n M : ℕ) : (Q n M).natDegree = dim n := by
  sorry

/-- Proposition 2.2: `G_K(ζ(5))` is a positive definite Gram matrix, so `Q_{K,M}(ζ(5)) > 0`. -/
theorem QKM_pos (n M : ℕ) : 0 < aeval (riemannZeta 5).re (Q n M) := by
  sorry
